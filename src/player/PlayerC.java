/**
 * 
 * Player subclass for Center Players. It initializes weight values and defines the method calculateScore()
 * by using random.nextGaussian(). The multiplier of the random.nextGaussian() is the standard deviation and the added
 * part is mean. By doing that, I almost guaranteed that every random number to be larger than 0. 
 * The other player subclass have the same logic, the only difference is weight values.
 */
...
public class PlayerC extends Player{
	...
	protected double trbW = 0.3;
	protected double astW = 0.1;
	protected double blkW = 0.3;
	protected double stlW = 0.1;
	
	
	public PlayerC(String playerName, double pts, double trb, double ast, double blk, double stl) {
		super(playerName, "C", pts, trb, ast, blk, stl);
